...
export default async function handler(req, res) {
  ...
  // SigiloPay credentials
  const PUBLIC_KEY = process.env.SIGILOPAY_PUBLIC_KEY;
  const SECRET_KEY = process.env.SIGILOPAY_SECRET_KEY;

  if (!PUBLIC_KEY || !SECRET_KEY) {
    console.error('[SigiloPay] Error: Keys are missing in environment variables.');
    return res.status(500).json({ success: false, error: 'Chaves da API SigiloPay não configuradas.' });
  }
  ...
  let URL;
  if (type === 'boleto') {
    URL = `${BASE_URL}/boleto/receive`;
  } else if (type === 'card') {
    URL = `${BASE_URL}/card/receive`;
  } else if (type === 'pix') {
    URL = `${BASE_URL}/pix/receive`;
  } else {
    return res.status(400).json({ error: 'Invalid payment type. Use "boleto", "card" or "pix".' });
  }
  ...
  try {
    ...
    // Robust Validation/Fallback for Client Data
    ...
    console.log('[SigiloPay] Sending payload:', JSON.stringify(finalPayload, null, 2));
    ...
    console.error('[SigiloPay] Server Error:', error);
